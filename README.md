...
| **Allure** | Test reporting (HTML report generation) | `allure-report/`, `allure-results/` |
...
The project uses **[Allure](https://allure-report.io/)** for test reporting (via `allure-playwright`) alongside the default HTML reporter.
...
### Allure output structure
| Directory | Contents |
|-----------|----------|
| `allure-results/` | Raw test data (result.json, attachments) — generated by `allure-playwright` |
| `allure-report/` | Final static HTML report — generated by `allure generate` |

The Allure report shows:
- **Overview dashboard** — total/passed/failed tests, duration, retries
- **Suites** — test spec-wise breakdown (login, create, get, patch, put, delete)
- **Timeline & Environment** — execution history and environment info
- **Behaviors** — grouped by feature/epic (if annotations used)
